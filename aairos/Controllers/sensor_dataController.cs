...
namespace aairos.Controllers
{
    ...
    public class sensor_dataController : ControllerBase
    {
        ...
        // GET: api/sensor_data/User/{deviceId}
        [HttpGet("User/{deviceId}")]
        public async Task<ActionResult<IEnumerable<SensorDataDto>>> GetLast2SensorDataByDeviceId(int deviceId)
        {
            try
            {
                var last2Data = await _context.sensor_data
                ...
                if (!last2Data.Any())
                {
                    return NotFound("No data found for the given deviceId.");
                }
        ...
        public async Task<IActionResult> ExportToCsv(int userProfileId, int deviceId, DateTime startDate, DateTime endDate)
        {
            try
            {
                ...
                // If startDate and endDate are the same, adjust endDate to include the full day
                if (startDate.Date == endDate.Date)
                {
                    endDate = endDate.Date.AddDays(1).AddTicks(-1); // End of the day
                }

                // Adjust endDate to include the current time if it matches today's date
                if (endDate.Date == DateTime.UtcNow.Date)
                {
                    endDate = DateTime.UtcNow; // Use the current UTC time for filtering
                }
                ...
                var filteredData = data
                    ...
                    {
                        record.deviceId,
                        TimestampMinute = new DateTime(
                            record.timestamp.Year,
                            record.timestamp.Month,
                            record.timestamp.Day,
                            record.timestamp.Hour,
                            record.timestamp.Minute / 15 * 15, // Calculate the nearest lower 15-minute interval 
                            0) // Truncate to the minute level
                    })
                    .Select(group => group.First()) // Select the first record from each group
                    .OrderBy(record => record.timestamp) // Ensure ordering is preserved
                    .ToList();
